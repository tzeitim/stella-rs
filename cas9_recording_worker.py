...
class Cas9RecordingWorker:
    ...
    def __init__(self, gt_tree_path: str, tier: int, output_dir: str, shared_dir: str):
    ...
    def apply_cas9_recording(self):
        ...
        try:
            ...
            # Save Cas9 instance to configured directory
            ...
            self.cas9_instance_path = cas9_instances_dir / f"tier{self.tier}_instance.pkl"
            ...
            # Update status
            self.update_status("level2", f"tier{self.tier}_recording", "completed", {
            ...
            self.update_status("level2", f"tier{self.tier}_recording", "failed", str(e))
            raise
    ...
    def submit_reconstruction_jobs(self) -> None:
        ...
        self.update_status("level3", f"tier{self.tier}_reconstructions", "submitted", {
            'cas9_instance_path': str(self.cas9_instance_path),
            'tier': self.tier,
    ...
    def submit_reconstruction_job(self, solver: str) -> str:
        ...
        cmd = [
            'bsub',
            '-J', f'reconstruct_tier{self.tier}_{solver}',
            '-oo', f"{self.shared_dir.resolve()}/logs/reconstruct_tier{self.tier}_{solver}_%J.out",
            '-eo', f"{self.shared_dir.resolve()}/logs/reconstruct_tier{self.tier}_{solver}_%J.err",
            '-W', '0:45',  # 45 minutes
            '-n', '3', '-R', 'span[hosts=1]',
            '-R', 'rusage[mem=1.5GB]',

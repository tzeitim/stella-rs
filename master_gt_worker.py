...
class MasterGTWorker:
    ...
    def generate_gt_tree(self) -> str:
        """Generate the ground truth tree."""
        logger.info("Generating ground truth tree...")
        ...
        try:
            ...
            gt_output_dir = Path(config_shared_dir)
            gt_output_dir.mkdir(parents=True, exist_ok=True)
            self.gt_tree_path = gt_output_dir / "gt_tree.pkl"
            
            # Save the GT tree
            with open(self.gt_tree_path, 'wb') as f:
                pickle.dump(gt_tree, f)
                
            logger.info(f"GT tree saved to: {self.gt_tree_path}")
            
            # Update status
            self.update_status("level1", "gt_generation", "completed")
            
            return str(self.gt_tree_path)
            
        except Exception as e:
            logger.error(f"Failed to generate GT tree: {e}")
            self.update_status("level1", "gt_generation", "failed", str(e))
            raise
    
    def submit_cas9_jobs(self) -> None:
        """Submit LSF jobs for all Cas9 tiers."""
        logger.info("Submitting Cas9 recording jobs for all tiers...")
        ...
        for tier_num in tier_nums:
            try:
                job_id = self.submit_cas9_job(tier_num)
                submitted_jobs.append({
                    'tier': tier_num,
                    'job_id': job_id,
                    'status': 'submitted'
                })
                logger.info(f"Submitted Cas9 Tier {tier_num} job: {job_id}")
                
            except Exception as e:
                logger.error(f"Failed to submit Cas9 Tier {tier_num} job: {e}")
                submitted_jobs.append({
                    'tier': tier_num,
                    'job_id': None,
                    'status': 'failed',
        ...
        self.update_status("level2", "cas9_jobs", "submitted", {
            'gt_tree_path': str(self.gt_tree_path),
            'submitted_jobs': submitted_jobs
        })
        
    def submit_cas9_job(self, tier: int) -> str:
        """Submit LSF job for a specific Cas9 tier."""
        ...
        cmd = [
            'bsub',
            '-J', f'cas9_tier{tier}_analysis',
            '-oo', f"{self.shared_dir.resolve()}/logs/cas9_tier{tier}_%J.out",
            '-eo', f"{self.shared_dir.resolve()}/logs/cas9_tier{tier}_%J.err",
            '-W', '0:30',  # 30 minutes
            '-n', '15', '-R', 'span[hosts=1]',
            '-R', 'rusage[mem=1.5GB]',
    ...
    def run(self) -> None:
        ...
        try:
            # Step 1: Generate GT tree
            self.generate_gt_tree()
            
            # Step 2: Submit Cas9 recording jobs
            self.submit_cas9_jobs()

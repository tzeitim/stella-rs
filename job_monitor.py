...
class JobMonitor:
    ...
    def _build_expected_structure(self) -> Dict[str, List[str]]:
        """Build expected job structure from configuration."""
        structure = {
            'level1': ['gt_generation'],
            'level2': [],
            'level3': []
        }
        
        # Get number of GT instances
        ...
        # Build level 2 (CAS9 recording) components - multiply by instances
        for instance in range(num_gt_instances):
            for tier in active_tiers:
                structure['level2'].append(f'instance{instance}_tier{tier}_recording')
        
        # Build level 3 (reconstruction) components - multiply by instances  
        for instance in range(num_gt_instances):
            for tier in active_tiers:
                for solver in active_solvers:
                    structure['level3'].append(f'instance{instance}_tier{tier}_{solver}')
    ...
    def check_file_outputs(self) -> Dict[str, Any]:
        ...
        # Check for GT tree - it can be in shared/ or shared/cas9_analysis/gt_trees/
        gt_tree_path1 = self.shared_dir.parent / "gt_tree.pkl"  # shared/gt_tree.pkl
        gt_tree_path2 = self.shared_dir / "gt_trees" / "gt_tree.pkl"  # shared/cas9_analysis/gt_trees/gt_tree.pkl
        
        file_status = {
            'gt_tree': gt_tree_path1.exists() or gt_tree_path2.exists(),
            'cas9_instances': {},
            'results': {}
        }
        
        # Check Cas9 instances - they can be in shared/cas9_instances/ or shared/cas9_analysis/cas9_instances/
        for tier in range(1, 5):
            cas9_file1 = self.shared_dir.parent / "cas9_instances" / f"tier{tier}_instance.pkl"
            cas9_file2 = self.shared_dir / "cas9_instances" / f"tier{tier}_instance.pkl"
            file_status['cas9_instances'][f'tier{tier}'] = cas9_file1.exists() or cas9_file2.exists()
        
        # Check results - they can be in shared/results/ or shared/cas9_analysis/results/
        active_solvers = self.config.get('solvers', {}).get('enabled', ['nj', 'maxcut', 'greedy', 'vanilla'])
        for tier in range(1, 5):
            for solver in active_solvers:
                result_file1 = self.shared_dir.parent / "results" / f"tier{tier}_{solver}_metrics.json"
                result_file2 = self.shared_dir / "results" / f"tier{tier}_{solver}_metrics.json"
                file_status['results'][f'tier{tier}_{solver}'] = result_file1.exists() or result_file2.exists()
    ...
    def generate_status_summary(self) -> Dict[str, Any]:
        ...
        # GT tree (1 item per instance)
        total_expected += num_gt_instances
        if file_outputs['gt_tree']:
            completed_count += 1  # Currently only tracking single GT tree
            
        # CAS9 instances (4 tiers × num_instances)
        total_expected += 4 * num_gt_instances
        completed_count += sum(1 for exists in file_outputs['cas9_instances'].values() if exists)
        
        # Results (dynamic based on active solvers, tiers, and instances)
        active_solvers = self.config.get('solvers', {}).get('enabled', ['nj', 'maxcut', 'greedy', 'vanilla'])
        expected_results = 4 * len(active_solvers) * num_gt_instances  # 4 tiers × N solvers × N instances
    ...
    def print_status_report(self, summary: Dict[str, Any]) -> None:
        ...
        print(f"\n📁 FILE OUTPUTS")
        print(f"   GT Tree: {'✅' if files['gt_tree'] else '❌'}")
        
        cas9_completed = sum(1 for exists in files['cas9_instances'].values() if exists)
        print(f"   Cas9 Instances: {cas9_completed}/4 tiers")
        
        results_completed = sum(1 for exists in files['results'].values() if exists)
        expected_results = len(summary['levels']['level3']['expected_components'])
        print(f"   Result Files: {results_completed}/{expected_results} reconstructions")
    ...
    def consolidate_parquet_results(self) -> None:
        ...
        try:
            ...
            results_dir1 = self.shared_dir.parent / "results"
            if results_dir1.exists():
                parquet_files.extend(results_dir1.glob("*.parquet"))
                
            # Check shared/cas9_analysis/results/  
            results_dir2 = self.shared_dir / "results"

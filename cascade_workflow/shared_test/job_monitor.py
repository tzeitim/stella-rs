...
class JobMonitor:
    ...
    def _build_expected_structure(self) -> Dict[str, List[str]]:
        ...
        # Build level 3 (reconstruction) components - multiply by instances, simulations, tiers, solvers  
        for instance in range(num_gt_instances):
            for sim in range(cas9_simulations_per_gt):
                for tier in active_tiers:
                    for solver in active_solvers:
                        structure['level3'].append(f'instance{instance}_sim{sim}_tier{tier}_{solver}')
                ...
                for tier in range(1, 5):
                    for solver in active_solvers:
                        ...
                        result_file_paths = [
                            ...
                            # Legacy naming without instance/simulation (current pattern)
                        ...
                        file_status['results'][f'instance{instance}_sim{sim}_tier{tier}_{solver}'] = any(path.exists() for path in result_file_paths)
